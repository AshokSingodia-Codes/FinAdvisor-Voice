...
verifier_chain = verifier_prompt | get_structured_chat(Verification)

def verify_answer(state: AgentState):
    print("---NODE: VERIFIER (BYPASSED FOR SPEED)---")
    draft = state["draft_answer"]
    
    # Bypass verification LLM call to save 3-4 seconds per query
    # and allow the bot to answer general knowledge questions.
    return {

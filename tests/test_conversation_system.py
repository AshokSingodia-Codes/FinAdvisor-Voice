...
def test_conversation_crud_endpoints():
    # 1. Create conversation
    res = client.post("/api/conversations", json={"title": "Test Budgeting"})
    ...
    res = client.patch(f"/api/conversations/{conv_id}", json={"title": "Updated Budgeting Title"})
    assert res.status_code == 200
...
def test_multi_turn_financial_memory_and_isolation():
    """
    ...
    conv_data = client.get(f"/api/conversations/{conv_id_a}").json()

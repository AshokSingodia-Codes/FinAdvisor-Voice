...
def test_indian_stock_query(auth_headers):
...
def test_student_advisor_query(auth_headers):
    ...
    assert "budget" in answer.lower() or "emergency" in answer.lower() or "save" in answer.lower() or "invest" in answer.lower() or "income" in answer.lower()

def test_company_comparison(auth_headers):
    conv_id = str(uuid.uuid4())
    response = client.post("/api/chat", json={

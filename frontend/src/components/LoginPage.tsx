...
export const LoginPage: React.FC = () => {
  ...
  // Sign up step: 1 = Email (Send OTP), 2 = Enter OTP + Passwords (Create Account)
  const [signupStep, setSignupStep] = useState<1 | 2>(1);

  // Forgot password step: 1 = Email (Send OTP), 2 = Enter OTP (Verify), 3 = Enter New Password
  ...
  // -------------------------------------------------------------
  // 4. CREATE ACCOUNT (Sign Up step 2)
  // -------------------------------------------------------------
  ...
  return (
    <div
      className="relative min-h-screen w-full flex items-center justify-center p-4 sm:p-6 bg-cover bg-center bg-no-repeat overflow-x-hidden font-sans"
      style={{
        backgroundImage: `url(${heroImage})`,
      }}
    >
      {/* Dark / Navy Tint Overlay with subtle backdrop blur */}
      <div className="absolute inset-0 bg-[#060a14]/75 backdrop-blur-[3px]" />

      {/* Subtle radial ambient glow in center */}
      <div className="absolute inset-0 bg-gradient-to-t from-[#060a14] via-transparent to-[#060a14]/60 pointer-events-none" />

      {/* =========================================================
          CENTERED GLASSMORPHISM CARD
      ========================================================= */}
      <div className="relative z-10 w-full max-w-[440px] bg-[#0c1322]/85 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl p-7 sm:p-9 text-slate-100 transition-all duration-300">
        ...
        {/* Dynamic Alerts */}
        {errorMessage && (
          <div className="mb-5 p-3 rounded-xl bg-red-500/10 border border-red-500/30 flex items-start gap-2.5 text-xs text-red-300 animate-in fade-in duration-200">
            <AlertCircle size={16} className="shrink-0 mt-0.5 text-red-400" />
            <span>{errorMessage}</span>
          </div>
        )}
        {successMessage && (
          <div className="mb-5 p-3 rounded-xl bg-cyan-500/10 border border-cyan-500/30 flex items-start gap-2.5 text-xs text-cyan-300 animate-in fade-in duration-200">
